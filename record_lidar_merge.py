...
def run_simulation(args, client):
    ...
    lidar_front= LiDAR(world=world, transform=transform_front,
                    attached=vehicle, 
                    sensor_options=sensor_options, verbose=False)
    ...
    lidar_merge = LiDARMerge(sensor_list=[lidar_front,lidar_left,lidar_back,lidar_right],world=world,
                    ...
                    attached=vehicle, verbose=args.verbose,
                    callbacks=[SceneExporter(ca_manager.actors)])
    frame = 0 
    if display_manager:
        display_manager.start()
    try:
        while is_running(display_manager, pcd_visualizer) and frame < args.number_of_frames:

            if display_manager:
                display_manager.check_events()
            if pcd_visualizer:
                pcd_visualizer.render()
            merged = lidar_merge.get_scene()
            if args.sync:
                world.tick()
            else:
                world.wait_for_tick()
...
def main():
    argparser = argparse.ArgumentParser(description='CARLA Record LiDAR Merge')
    ...
    argparser.set_defaults(render=False)
    argparser.add_argument('--verbose', action='store_true', help='print debug information (default: False)')
    argparser.set_defaults(verbose=False)
    argparser.add_argument('--number-of-frames', metavar='N', default=100, type=int, help='number of frames to record (default: 100)')
    argparser.add_argument('--output-dir', metavar='DIR', default='./data', help='output directory (default: output)')
    args = argparser.parse_args()


    args.width, args.height = [int(x) for x in args.res.split('x')]
    # Number of frames to record
    print('Number of frames to record: {}'.format(args.number_of_frames))
